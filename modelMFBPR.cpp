...
void ModelMFBPR::gradCheck() {
  ...
  float funcValPlus =
      -1 * std::log(1.0 / (1.0 + std::exp(-(r_ui_est - r_uj_est))));
  ...
  std::cout << funcVal << " " << funcValPlus << " " << funcValMinus
            << std::endl;
  std::cout << grad(0) << " " << approxGrad << " " << diff << std::endl;
}

std::vector<std::tuple<int, int, float>>
ModelMFBPR::getBPRUIRatings(gk_csr_t *mat) {
...
PosNegPair
ModelMFBPR::samplePosNegItem(int u, const gk_csr_t *trainMat,
    ...
    while (nTry < 50) {
      ...
      if (ui_rowval[jj + ui_rowptr[u]] < posRat) {
        ...
        if (0 == jj) {
          ...
          start =
              ui_rowind[ui_rowptr[u] + jj] + 1; // item next to last rated item
          end = nTrainItems;
        } else {
  ...
  while (nTry < 50) {
    ...
    if (ui_rowval[jj + ui_rowptr[u]] < posRat) {
      ...
      if (0 == jj) {
        ...
        start = ui_rowind[ui_rowptr[u] + jj] + 1; // item next to last rated
                                                  // item
        end = nTrainItems;
      } else {
        start = ui_rowind[ui_rowptr[u] + jj] + 1; // item next to jjth item
...
#pragma omp parallel for reduction(+ : trainLoss, nTrainInversions)
    for (int z = 0; z < uiRatingInds.size(); z++) {
      ...
      for (int i = 0; i < facDim; i++) {
        iFac(pI, i) -=
            learnRate * ((expCoeff * uFac(u, i)) + 2.0 * iReg * iFac(pI, i));
        iFac(nI, i) -=
            learnRate * ((-expCoeff * uFac(u, i)) + 2.0 * iReg * iFac(nI, i));
      }
    }
    ...
    if (!std::isfinite(trainLoss)) {
      std::cout << "Training loss is not finite (decrease learn rate): "
                << trainLoss << std::endl;
      exit(0);
    }
    ...
    if (iter % OBJ_ITER == 0 || iter == maxIter - 1) {
      // if (isTerminateModelNDCG(bestModel, data, iter, bestIter, bestHR,
      // valHR,
    ...
    if (iter % DISP_ITER == 0) {
      std::cout << "ModelMFBPR::trainHog trainSeed: " << trainSeed
                ...
                << " trainLoss: " << trainLoss
                << " subIterDuration: " << subIterDuration << std::endl;
    }
    */
  ...
  std::cout << "\nBest model validation NDCG: "
            << bestModel.NDCG(invalidUsers, invalidItems, data.valMat)
            << std::endl;
}
...
#pragma omp parallel for reduction(+ : trainLoss, nTrainInversions)
      for (int ind = 0; ind < trainUsers.size(); ind++) {
        ...
        if (!std::isfinite(expCoeff) || !std::isfinite(trainLoss)) {
          std::cout
              << "Gradient/trainLoss is not finite (decrease learn rate): "
              << expCoeff << " " << trainLoss << std::endl;
          exit(0);
        }
