...
int main(int argc, char *argv[]) {
  ...
  */
  ...
  std::cout << "nUsers with ratings: " << nzUsersItems.first.size() <<
  std::endl; std::cout << "nItems with ratings: " << nzUsersItems.second.size()
  << std::endl; std::cout << "nnz: " << nnz << std::endl;
  ...
  */
  ...
  // writeTailTestMat(data.testMat, "nf_480189x17772.tail.test.5.csr",
  // headItems);
...
#pragma omp parallel for
  ...
  */
  ...
  //   "", ,  params.seed);
  // writeItemSimMat(data.trainMat,
  // "ratings_26779x26779_25.syn.trainItems.metis");
  // writeItemSimMatNonSymm(data.trainMat,
  //    "ratings_26779x26779_25.syn.trainItems.nonsym.metis");

  // std::string graphFName = params.prefix +
  // std::string(".train.jacSim.metis"); writeItemJaccSimMat(data.trainMat,
  // graphFName.c_str());
  ...
  */
  ...
  data.valMat = mats[0];
  gk_csr_CreateIndex(data.valMat, GK_CSR_COL);
  std::cout << "val nusers: " << data.valMat->nrows << " nItems: " <<
  data.valMat->ncols << std::endl; data.graphMat = mats[1];
  gk_csr_CreateIndex(data.graphMat, GK_CSR_COL);

  std::cout << "graph nusers: " << data.graphMat->nrows << " nItems: " <<
  data.graphMat->ncols << std::endl;
  */
  ...
  */
  ...
  // mfModel.trainSGDProbOrderedPar(data, bestModel, invalidUsers,
  // invalidItems); mfModel.trainSGDProbPar(data, bestModel, invalidUsers,
  // invalidItems); mfModel.trainSGDOnlyOrderedPar(data, bestModel,
  // invalidUsers, invalidItems); mfModel.train(data, bestModel, invalidUsers,
  // invalidItems);
