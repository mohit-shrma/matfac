...
public:
  ...
  Params(int facDim, int maxIter, int svdFacDim, int seed, float uReg,
         float iReg, float learnRate, float rhoRMS, float alpha,
         std::string &trainMatFile, std::string &testMatFile,
         std::string &valMatFile, std::string &graphMatFile,
         std::string &negMatFile, std::string &origUFacFile,
         std::string &origIFacFile, std::string &initUFacFile,
         std::string &initIFacFile, std::string &prefix)
      : nUsers(-1), nItems(-1), facDim(facDim), maxIter(maxIter),
        svdFacDim(svdFacDim), seed(seed), uReg(uReg), iReg(iReg),
        learnRate(learnRate), rhoRMS(rhoRMS), alpha(alpha),
        trainMatFile(trainMatFile.c_str()), testMatFile(testMatFile.c_str()),
        valMatFile(valMatFile.c_str()),
        graphMatFile(graphMatFile.empty() ? NULL : graphMatFile.c_str()),
        negMatFile(negMatFile.empty() ? NULL : negMatFile.c_str()),
  ...
  void display() {
    ...
    std::cout << "facDim: " << facDim << " svdFacDim: " << svdFacDim
              << std::endl;
    ...
    std::cout << "graphMat: " << (NULL != graphMatFile ? graphMatFile : " ")
              << std::endl;
    std::cout << "negMat: " << (NULL != negMatFile ? negMatFile : " ")
              << std::endl;
    std::cout << "origUFac: " << (NULL != origUFacFile ? origUFacFile : " ")
              << std::endl;
    std::cout << "origIFac: " << (NULL != origIFacFile ? origIFacFile : " ")
              << std::endl;
    std::cout << "initUFac: " << (NULL != initUFacFile ? initUFacFile : " ")
              << std::endl;
    std::cout << "initIFac: " << (NULL != initIFacFile ? initIFacFile : " ")
              << std::endl;
  }
};
