...
std::string Model::modelSignature() {
  ...
  std::string sign = std::to_string(nUsers) + "X" + std::to_string(nItems) +
                     "_" + std::to_string(facDim) + "_" + std::to_string(uReg) +
                     "_" + std::to_string(iReg) + "_" +
                     std::to_string(origLearnRate);
...
void Model::load(const char *uFacName, const char *iFacName,
                 const char *uBFName, const char *iBFName,
                 const char *gBFName) {
  // read user latent factors
...
std::pair<int, double> Model::RMSE(gk_csr_t *mat,
...
std::pair<int, double> Model::SE(gk_csr_t *mat,
                                 std::unordered_set<int> &filtItems,
                                 std::unordered_set<int> &invalidUsers,
                                 std::unordered_set<int> &invalidItems) {
...
std::pair<int, double> Model::RMSEU(gk_csr_t *mat,
...
double Model::subMatRMSE(gk_csr_t *mat, int uStart, int uEnd, int iStart,
                         int iEnd) {
...
double Model::subMatExRMSE(gk_csr_t *mat, int uStart, int uEnd, int iStart,
                           int iEnd) {
  double r_ui_est, diff, rmse, r_ui;
  int u, ii, item, nnz;
  ...
  if (iter > 0) {
    ...
    if (iter - bestIter >= 50) {
      ...
      printf("\nNOT CONVERGED: bestIter:%d bestObj: %.10e"
             " currIter:%d currObj: %.10e",
             bestIter, bestObj, iter, currObj);
      ret = true;
    }
...
std::pair<double, double> Model::arHRI(const Data &data,
                                       std::unordered_set<int> &filtItems,
                                       std::unordered_set<int> &invalidUsers,
                                       std::unordered_set<int> &invalidItems,
                                       gk_csr_t *testMat) {
...
std::pair<double, double> Model::arHRU(const Data &data,
                                       std::unordered_set<int> &filtUsers,
                                       std::unordered_set<int> &invalidUsers,
                                       std::unordered_set<int> &invalidItems,
                                       gk_csr_t *testMat) {
...
std::pair<int, double> Model::hitRateU(const Data &data,
                                       std::unordered_set<int> &filtUsers,
                                       std::unordered_set<int> &invalidUsers,
                                       std::unordered_set<int> &invalidItems,
                                       gk_csr_t *testMat) {
...
bool Model::isTerminateModel(
    Model &bestModel, const Data &data, int iter, int &bestIter,
    double &bestObj, double &prevObj, std::unordered_set<int> &invalidUsers,
    std::unordered_set<int> &invalidItems,
...
bool Model::isTerminateModel(Model &bestModel, const Data &data, int iter,
  ...
  if (fabs(prevObj - currObj) < EPS) {
    ...
    printf("\nConverged in iteration: %d prevObj: %.10e currObj: %.10e"
           " bestValRMSE: %.10e",
           iter, prevObj, currObj, bestValRMSE);
    ret = true;
  }
...
bool Model::isTerminateModelSing(Model &bestModel, const Data &data, int iter,
                                 int &bestIter, double &bestObj,
                                 double &prevObj, double &bestValRMSE,
                                 double &prevValRMSE,
                                 std::unordered_set<int> &invalidUsers,
                                 std::unordered_set<int> &invalidItems) {
  bool ret = false;
...
bool Model::isTerminateModelSubMat(Model &bestModel, const Data &data, int iter,
                                   int &bestIter, double &bestObj,
                                   double &prevObj, int uStart, int uEnd,
                                   int iStart, int iEnd) {
  bool ret = false;
...
bool Model::isTerminateModelExSubMat(Model &bestModel, const Data &data,
                                     int iter, int &bestIter, double &bestObj,
                                     double &prevObj, int uStart, int uEnd,
                                     int iStart, int iEnd) {
  bool ret = false;
...
double
Model::objective(const Data &data, std::unordered_set<int> &invalidUsers,
                 std::unordered_set<int> &invalidItems,
                 std::vector<std::tuple<int, int, float>> &trainRatings) {
...
double Model::objectiveSing(const Data &data,
                            std::unordered_set<int> &invalidUsers,
                            std::unordered_set<int> &invalidItems) {
...
double Model::subMatKnownRankNonObsErr(const Data &data, int uStart, int uEnd,
  ...
  rmseUnknown =
      sqrt(seUnknown / (((uEnd - uStart) * (iEnd - iStart)) - nnzKnown));

  return rmseUnknown;
}

// start inclusive, end exclusive
double Model::subMatKnownRankNonObsErrWSet(const Data &data, int uStart,
...
#pragma omp parallel for
  for (int item = 0; item < nItems; item++) {
    itemFreq[item] =
        (data.trainMat->colptr[item + 1] - data.trainMat->colptr[item]);
  }
...
Model::Model(int nUsers, int nItems, int facDim)
    : nUsers(nUsers), nItems(nItems), facDim(facDim) {}
