...
void writeItemFreqSE(int user, std::vector<std::pair<int, double>> &scorePairs,
                     std::map<int, double> itemFreqMap, Model &origModel,
                     Model &fullModel, std::ofstream &itemOpFile,
                     std::ofstream &freqOpFile, std::ofstream &seOpFile) {
  int item;
...
void compMovAvg(std::vector<double> &slidingAvgRMSEs,
                std::vector<double> &slidingAvgScore,
                std::vector<std::pair<int, double>> &scorePairs, int user,
                int size, int width, Model &origModel, Model &fullModel) {
  ...
  for (int i = 0; i < nValues; i++) {
    ...
    if (0 == i) {
      for (int j = start; j < end; j++) {
        ...
        diff =
            origModel.estRating(user, item) - fullModel.estRating(user, item);
        se += diff * diff;
      }
    } else {
...
void compMovAvg2(std::vector<double> &slidingAvgRMSEs,
                 std::vector<double> &slidingAvgScore,
                 std::vector<std::pair<int, double>> &scorePairs, int user,
                 int size, int width, Model &origModel, Model &fullModel) {
...
void compMovAvg3(std::vector<double> &slidingAvgRMSEs,
                 std::vector<double> &slidingAvgScore,
                 std::vector<double> &slidingAvgFreq,
                 std::vector<double> &slidingAvgGt,
                 std::vector<std::pair<int, double>> &scorePairs,
                 std::map<int, double> &itemFreqMap, int user, int size,
...
void updateBucketsArr(std::vector<double> &bucketScores,
                      std::vector<double> &bucketNNZ, std::vector<double> &arr,
                      int nBuckets) {
  int nItems = arr.size();
...
uIGraphItemScores(int user, gk_csr_t *graphMat, float lambda, int nUsers,
...
std::vector<std::pair<int, double>>
zero1Scale(std::vector<std::pair<int, double>> &pairScores) {
...
std::vector<std::pair<int, double>>
avgScores(std::vector<std::pair<int, double>> &pairScores1,
  ...
  for (auto &&pairScore : pairScores2) {
    ...
    if (mapScores1.find(key) != mapScores1.end()) {
      resScores.push_back(
          std::make_pair(key, ((.95 * mapScores1[key]) + (0.05 * score))));
    }
  }
...
std::vector<std::pair<int, double>>
prodScores(std::vector<std::pair<int, double>> &pairScores1,
           std::vector<std::pair<int, double>> &pairScores2) {
...
// x + alpha log(y)
std::vector<std::pair<int, double>>
logSumScores(std::vector<std::pair<int, double>> &pairScores1,
  ...
  for (auto &&pairScore : pairScores2) {
    ...
    if (mapScores1.find(key) != mapScores1.end()) {
      resScores.push_back(
          std::make_pair(key, mapScores1[key] + (alpha * log(score))));
    }
  }
...
// x - alpha log(y)
std::vector<std::pair<int, double>>
logDiffScores(std::vector<std::pair<int, double>> &pairScores1,
              std::vector<std::pair<int, double>> &pairScores2, double alpha) {
...
// log(x) + alpha log(y)
std::vector<std::pair<int, double>>
logLogSumScores(std::vector<std::pair<int, double>> &pairScores1,
                std::vector<std::pair<int, double>> &pairScores2,
                double alpha) {
  ...
  for (auto &&pairScore : pairScores2) {
    ...
    if (mapScores1.find(key) != mapScores1.end()) {
      resScores.push_back(
          std::make_pair(key, log(mapScores1[key]) + (alpha * log(score))));
    }
  }
...
// log(x) - alpha log(y)
std::vector<std::pair<int, double>>
logLogDiffScores(std::vector<std::pair<int, double>> &pairScores1,
                 std::vector<std::pair<int, double>> &pairScores2,
                 double alpha) {
...
std::vector<std::pair<int, double>>
itemGraphItemScores(int user, gk_csr_t *graphMat, gk_csr_t *mat, float lambda,
                    int nUsers, int nItems, std::unordered_set<int> &invalItems,
                    bool useRatings) {
...
std::vector<std::pair<int, double>>
itemSVDScores(Model &svdModel, int user, gk_csr_t *mat, int nItems,
...
std::vector<std::pair<int, double>>
itemOptScores(Model &origModel, Model &fullModel, int user, gk_csr_t *mat,
              int nItems, std::unordered_set<int> &invalItems) {
...
std::vector<std::pair<int, double>>
itemPredScores(Model &fullModel, int user, gk_csr_t *mat, int nItems,
               std::unordered_set<int> &invalItems) {
...
std::vector<std::pair<int, double>>
itemOrigScores(Model &origModel, int user, gk_csr_t *mat, int nItems,
               std::unordered_set<int> &invalItems) {
...
std::vector<std::pair<int, double>>
itemFreqScores(Model &fullModel, Model &origModel, std::vector<double> itemFreq,
               int user, gk_csr_t *mat, int nItems,
               std::unordered_set<int> &invalItems) {
...
std::vector<std::pair<int, double>>
iterSort(std::vector<std::pair<int, double>> &itemScoresPair1,
...
void compOverlapPcLaterBins(std::vector<std::pair<int, double>> &predScorePairs,
                            ...
                            std::vector<double> &pcPredBins,
                            std::vector<double> &pcSVDBins, int topBuckN) {
  ...
  auto halfTopPredScorePairs = std::vector<std::pair<int, double>>(
      predScorePairs.begin() + (topBuckN / 2),
      predScorePairs.begin() + topBuckN);
...
void itemRMSEsProb(int user, gk_csr_t *graphMat, gk_csr_t *trainMat,
                   float lambda, int nUsers, int nItems,
                   std::unordered_set<int> &invalItems,
                   std::unordered_set<int> &filtItems, Model &fullModel,
                   Model &origModel, std::vector<double> &itemsRMSE,
                   std::vector<double> &itemsProb) {

  std::vector<std::pair<int, double>> itemScores = itemGraphItemScores(
      user, graphMat, trainMat, lambda, nUsers, nItems, invalItems, true);
...
void itemRMSEsOrdByItemScores(
    int user, std::unordered_set<int> &filtItems, Model &fullModel,
    Model &origModel, std::vector<double> &itemsRMSE,
...
void itemSignEsOrdByItemScores(
    int user, std::unordered_set<int> &filtItems, Model &fullModel,
    Model &origModel, std::vector<double> &itemsRMSE,
    std::vector<double> &itemsScore,
    std::vector<std::pair<int, double>> &itemScoresPair) {
...
void itemScoresFrmMap(std::vector<std::pair<int, double>> &itemScoresPair,
                      std::map<int, double> &itemScoreMap,
                      std::vector<double> &orderedScore) {
  orderedScore.clear();
...
void pprUsersRMSEProb(gk_csr_t *graphMat, gk_csr_t *trainMat, int nUsers,
                      int nItems, Model &origModel, Model &fullModel,
...
void pprSampUsersRMSEProb(gk_csr_t *graphMat, gk_csr_t *trainMat, int nUsers,
                          int nItems, Model &origModel, Model &fullModel,
                          float lambda, int max_niter,
                          std::unordered_set<int> &invalUsers,
                          std::unordered_set<int> &invalItems,
                          std::unordered_set<int> &filtItems, int nSampUsers,
  ...
  while (sampUsers.size() < nSampUsers) {
    ...
    if (sampUsers.size() % PROGU == 0) {
      std::cout << "Done... " << sampUsers.size() << " :" << lambda
                << std::endl;
    }
  }
...
void freqSampUsersRMSEProb(gk_csr_t *trainMat, int nUsers, int nItems,
...
void svdSampUsersRMSEProb(gk_csr_t *trainMat, int nUsers, int nItems,
  ...
  while (sampUsers.size() < nSampUsers) {
    ...
    auto itemScoresPair =
        itemSVDScores(svdModel, user, trainMat, nItems, invalItems);
...
void optSampUsersRMSEProb(gk_csr_t *trainMat, int nUsers, int nItems,
  ...
  while (sampUsers.size() < nSampUsers) {
    ...
    // get item scores ordered by SVD in decreasing order
    auto itemScoresPair =
        itemOptScores(origModel, fullModel, user, trainMat, nItems, invalItems);
...
void countInvRatings(Model &origModel, Model &fullModel, int user,
                     std::vector<std::pair<int, double>> itemPreds,
                     float avgRat, int &predLowcount, int &predHighcount,
                     int &lowCount, int &highCount, int N) {
...
std::vector<std::pair<int, double>>
orderingOverlap(std::vector<std::pair<int, double>> itemPairsA,
...
// items in B not in A
std::vector<std::pair<int, double>>
orderingDiff(std::vector<std::pair<int, double>> itemPairsA,
             std::vector<std::pair<int, double>> itemPairsB, int sizeA) {
  int overlapCount = 0;
...
compOrderingOverlapBScores(std::vector<std::pair<int, double>> itemPairsA,
...
std::pair<double, double>
compDiffPc(std::vector<std::pair<int, double>> &itemPairsA,
           std::vector<std::pair<int, double>> &itemPairsB) {
  int ovCount = 0;
...
void predSampUsersRMSEProb(gk_csr_t *trainMat, gk_csr_t *graphMat, int nUsers,
                           int nItems, Model &origModel, Model &fullModel,
                           Model &svdModel, std::unordered_set<int> &invalUsers,
                           std::unordered_set<int> &invalItems,
                           std::unordered_set<int> &filtItems, int nSampUsers,
                           int seed, std::string prefix) {
  ...
  while (sampUsers.size() < nSampUsers) {
    ...
    auto itemPredScoresPair =
        itemPredScores(fullModel, user, trainMat, nItems, invalItems);
    auto itemOrigScoresPair =
        itemOrigScores(origModel, user, trainMat, nItems, invalItems);
    auto itemSVDScoresPair =
        itemSVDScores(svdModel, user, trainMat, nItems, invalItems);
    // auto itemPPRScoresPair = itemGraphItemScores(user, graphMat, trainMat,
    //    0.01, nUsers, nItems, invalItems, true);

    auto itemFreqPair = itemFreqScores(fullModel, origModel, itemFreq, user,
                                       trainMat, nItems, invalItems);
    auto itemPredSVDScoresPair =
        prodScores(itemPredScoresPair, itemSVDScoresPair);
    // auto itemPredPPRScoresPair = prodScores(itemPredScoresPair,
    //    itemPPRScoresPair);
    auto svdItemPairsNotInPred =
        orderingDiff(itemPredScoresPair, itemSVDScoresPair, topBuckN);

    // auto pprItemPairsNotInPred = orderingDiff(itemPredScoresPair,
    //    itemPPRScoresPair, topBuckN);

    auto freqItemPairsNotInPred =
        orderingDiff(itemPredScoresPair, itemFreqPair, topBuckN);

    svdNotInPredOrigOverlap += compOrderingOverlap(
        itemOrigScoresPair, svdItemPairsNotInPred, topBuckN);
    // pprNotInPredOrigOverlap += compOrderingOverlap(itemOrigScoresPair,
    //    pprItemPairsNotInPred, topBuckN);
    freqNotInPredOrigOverlap += compOrderingOverlap(
        itemOrigScoresPair, freqItemPairsNotInPred, topBuckN);

    predOrigOverlap +=
        compOrderingOverlap(itemOrigScoresPair, itemPredScoresPair, topBuckN);
    svdOrigOverlap +=
        compOrderingOverlap(itemOrigScoresPair, itemSVDScoresPair, topBuckN);

    auto origOverScores = compOrderingOverlapBScores(
        itemOrigScoresPair, itemSVDScoresPair, topBuckN);
    ...
    freqOrigOverlap +=
        compOrderingOverlap(itemOrigScoresPair, itemFreqPair, topBuckN);
    ...
    svdPredOverlap +=
        compOrderingOverlap(itemSVDScoresPair, itemPredScoresPair, topBuckN);
    ...
    countInvRatings(origModel, fullModel, user, itemScoresPair, avgUserRating,
                    predLowcount, predHighcount, lowCount, highCount, topBuckN);
  ...
  std::cout << "\nbuck size: " << topBuckN
            ...
            << "\nlow rated Items pred high pc: "
            << (float)predHighcount / lowCount << "\nlow rated Items pc: "
            << (float)lowCount / ((float)topBuckN * sampUsers.size())
            << std::endl;

  std::cout << "\npredLoCount: " << (float)predLowcount
            << " highcount: " << highCount
            << " pc: " << (float)predLowcount / highCount
            << "\nhigh rated items pred low pc: "
            << (float)predLowcount / highCount << "\nhigh rated items pc: "
            << (float)highCount / ((float)topBuckN * sampUsers.size())
            << std::endl;

  std::cout << "predOrigOverlap: " << predOrigOverlap / sampUsers.size()
            << std::endl;
  std::cout << "svdOrigOverlap: " << svdOrigOverlap / sampUsers.size()
            << std::endl;
  std::cout << "freqOrigOverlap: " << freqOrigOverlap / sampUsers.size()
            << std::endl;
  std::cout << "freqNotInPredOrigOverlap: "
            << freqNotInPredOrigOverlap / sampUsers.size() << std::endl;
  // std::cout << "pprOrigOverlap: " << pprOrigOverlap/sampUsers.size() <<
  // std::endl; std::cout << "predPPROrigOverlap: " <<
  // predPPROrigOverlap/sampUsers.size() << std::endl;

  // std::cout << "pprNotInPredOrigOverlap: " <<
  // pprNotInPredOrigOverlap/sampUsers.size()
  //  << std::endl;
  std::cout << "svdPredOverlap: " << svdPredOverlap / sampUsers.size()
            << std::endl;
  std::cout << "svdNotInPredOrigOverlap: "
            << svdNotInPredOrigOverlap / sampUsers.size() << std::endl;
  std::cout << "predSVDOrigOverlap: " << predSVDOrigOverlap / sampUsers.size()
            << std::endl;
...
void updateMisPredBins(std::vector<double> &misPredCountBins,
                       ...
                       std::vector<std::pair<int, double>> &origScorePairs,
                       std::vector<std::pair<int, double>> &predNotInTopPairs,
                       int topBuckN, std::map<int, double> &pprScoreMap,
                       int user) {
...
void updateMisPredBins(std::vector<double> &misPredCountBins,
                       ...
                       std::vector<std::pair<int, double>> &origScorePairs,
                       std::vector<std::pair<int, double>> &predNotInTopPairs,
                       int topBuckN, Model &svdModel, int user) {
...
void updateMisPredFreqBins(
    std::vector<double> &misPredCountBins, std::vector<double> &freqScoreBins,
    std::vector<std::pair<int, double>> &origScorePairs,
...
void predSampUsersRMSEProb2(gk_csr_t *trainMat, gk_csr_t *graphMat, int nUsers,
                            int nItems, Model &origModel, Model &fullModel,
                            Model &svdModel,
                            std::unordered_set<int> &invalUsers,
                            std::unordered_set<int> &invalItems,
  ...
  for (int user = 0; user < nUsers; user++) {
    ...
    */
    ...
    svdAboveAvgInOrig +=
        compOrderingOverlap2(itemOrigScoresPair, aboveAvgSVDPairs, topBuckN);

    auto predInOrigTop =
        orderingOverlap(itemOrigScoresPair, itemPredScoresPair, topBuckN);
    double svdScore = 0;
    ...
    */
    ...
    if (predInOrigTop.size()) {
      ...
      svdOfMedPredInOrig += svdModel.estRating(
          user, predInOrigTop[predInOrigTop.size() / 2].first);
      svdOfTopPredInOrig += svdModel.estRating(user, predInOrigTop[0].first);
      svdOfBotPredInOrig += svdModel.estRating(
          user, predInOrigTop[predInOrigTop.size() - 1].first);
    }
    ...
    for (auto &&pair : predInOrigTop) {
      int item = pair.first;
      svdVarPredInOrig +=
          std::pow(svdModel.estRating(user, item) - svdScore, 2);
    }
    if (predInOrigTop.size()) {
    ...
    auto predNotInOrig =
        orderingDiff(itemOrigScoresPair, itemPredScoresPair, topBuckN);
    svdScore = 0;
    svdScores.clear();
    ...
    // updateMisPredFreqBins(misPredFreqCountBins, freqScoreBins,
    // itemOrigScoresPair,
    //    predNotInOrig, topBuckN, itemFreq, user);
    updateMisPredFreqBins(misPredFreqCountBins, freqScoreBins,
                          itemOrigScoresPair, itemPredTopN, topBuckN, itemFreq,
                          user);

    auto gtNotInPred =
        orderingDiff(itemPredScoresPair, itemOrigScoresPair, topBuckN);
    // updateMisPredBins(misGTSVDCountBins, misGTSVDScoreBins,
    // itemPredScoresPair,
    ...
    // updateMisPredFreqBins(misGTFreqCountBins, misGTFreqScoreBins,
    // itemPredScoresPair,
    //    gtNotInPred, topBuckN, itemFreq, user);
    updateMisPredFreqBins(misGTFreqCountBins, misGTFreqScoreBins,
                          itemPredScoresPair, itemOrigTopN, topBuckN, itemFreq,
                          user);
    ...
    updateMisPredFreqBins(misGTAvgTrainCountBins, misGTAvgTrainScoreBins,
                          itemPredScoresPair, itemOrigTopN, topBuckN,
                          itemAvgRatings, user);

    // updateMisPredBins(misGTOrigCountBins, misGTOrigScoreBins,
    // itemPredScoresPair,
    //    gtNotInPred, topBuckN, origModel, user);
    updateMisPredBins(misGTOrigCountBins, misGTOrigScoreBins,
                      itemPredScoresPair, itemOrigTopN, topBuckN, origModel,
                      user);
    ...
    */
    ...
    if (NULL != graphMat) {
      itemPPRScoresPair = itemGraphItemScores(
          user, graphMat, trainMat, 0.01, nUsers, nItems, invalItems, false);
      std::map<int, double> pprMap;
      for (auto &&itemScore : itemPPRScoresPair) {
        if (std::isnan(itemScore.second)) {
      ...
      // updateMisPredBins(misPredPPRCountBins, pprScoreBins,
      // itemOrigScoresPair,
      ...
      // updateMisPredBins(misGTPPRCountBins, misGTPPRScoreBins,
      // itemPredScoresPair,
      //    gtNotInPred, topBuckN, pprMap, user);
      updateMisPredBins(misGTPPRCountBins, misGTPPRScoreBins,
                        itemPredScoresPair, itemOrigTopN, topBuckN, pprMap,
                        user);
      ...
      pprOrigOverlap +=
          compOrderingOverlap(itemOrigScoresPair, itemPPRScoresPair, topBuckN);

      /*
      auto topPPRPairsFrmMF = iterSort(itemPredScoresPair, itemPPRScoresPair,
          topBuckN);

      compOverlapPcBins(topPPRPairsFrmMF, itemOrigScoresPair,
      svdOrderedPredBins, topBuckN);

      compOverlapPcBins(itemPredScoresPair, itemOrigScoresPair,
      mfOrderedPredBins, topBuckN);
      */
    }

    auto topSVDPairsFrmMF =
        iterSort(itemPredScoresPair, itemSVDScoresPair, topBuckN * 2);

    iterPredSVDOrigOverlap +=
        compOrderingOverlap(itemOrigScoresPair, topSVDPairsFrmMF, topBuckN);

    topSVDPairsFrmMF =
        iterSort(itemPredScoresPair, itemSVDScoresPair, topBuckN);
    ...
    */
    ...
    auto prodSVDAvgScoresPair =
        prodScores(itemPredScoresPair, itemSVDScoresPair);

    predSVDAvgOverlap +=
        compOrderingOverlap(itemOrigScoresPair, prodSVDAvgScoresPair, topBuckN);
  ...
  std::cout << "pprofPredInOrig: " << pprOfPredInOrig / sampUsers.size()
            << std::endl;
  std::cout << "pprOfPredNotInOrig: " << pprOfPredNotInOrig / sampUsers.size()
            << std::endl;
  ...
  std::cout << "svdOfPredInOrig: " << svdOfPredInOrig / sampUsers.size()
            << " svdOfPredNotInOrig: " << svdOfPredNotInOrig / sampUsers.size()
            << std::endl;

  std::cout << "svdVarOfPredInOrig: " << svdVarOfPredInOrig / sampUsers.size()
            << " svdVarOfPredNotInOrig: "
            << svdVarOfPredNotInOrig / sampUsers.size() << std::endl;

  std::cout << "svdAboveAvgInOrig: " << svdAboveAvgInOrig / sampUsers.size()
            << std::endl;

  std::cout << "svdOfMaxPredInorig: " << svdOfMaxPredInorig / sampUsers.size()
            << std::endl;
  std::cout << "svdOfMinPredInOrig: " << svdOfMinPredInOrig / sampUsers.size()
            << std::endl;
  std::cout << "svdOfTopPredInOrig: " << svdOfTopPredInOrig / sampUsers.size()
            << std::endl;
  std::cout << "svdOfMedPredInOrig: " << svdOfMedPredInOrig / sampUsers.size()
            << std::endl;
  std::cout << "svdOfBotPredInOrig: " << svdOfBotPredInOrig / sampUsers.size()
            << std::endl;
  ...
  */
  ...
  std::cout << "predSVDAvgOverlap: " << predSVDAvgOverlap / sampUsers.size()
            << std::endl;
...
void predSampUsersRMSEProbPar(const Data &data, int nUsers, int nItems,
  ...
  int nIntervals =
      100 - ((double)slidingWidth * 100) / (double)nItems + 1; //(size -k + 1)
  ...
  std::vector<int> sampUsersVec =
      std::vector<int>(sampUsers.begin(), sampUsers.end());
...
#pragma omp for reduction(+ : freqTopRMSE, svdTopRMSE, pprTopRMSE, freqSVDTopOvrlap, pprFreqTopOvrlap, pprSVDTopOvrlap, freqNOTinSVDPc, svdNOTinFreqPc, freqNOTinPPRPc, pprNOTinFreqPc, svdNOTinPPRPc, pprNOTinSVDPc, freqOrigOverlap, predOrigOverlap, svdOrigOverlap, svdPredOverlap, pprOrigOverlap, svdNotInPred, svdInPred, svdOfPredInOrig, svdOfPredNotInOrig, svdVarOfPredInOrig, svdVarOfPredNotInOrig, svdOfMedPredInOrig, svdOfMaxPredInorig, svdOfMinPredInOrig, svdOfTopPredInOrig, svdOfBotPredInOrig, svdAboveAvgInOrig, pprOfPredInOrig, pprOfPredNotInOrig, pprNotInPred, pprInPred, predPPROrigOverlap, iterPredSVDOrigOverlap, totalSampUsers, freq_SVDTopInFreqTop, freq_SVDTopNOTInFreqTop, freq_PPRTopInFreqTop, freq_PPRTopNOTInFreqTop, rmse_SVDTopInFreqTop, rmse_SVDTopNOTInFreqTop, rmse_PPRTopInFreqTop, rmse_PPRTopNOTInFreqTop)
    for (int uInd = 0; uInd < sampUsersSz; uInd++) {
      ...
      auto itemPredScoresPair =
          itemPredScores(fullModel, user, trainMat, nItems, uInvalItems);
      auto itemOrigScoresPair =
          itemOrigScores(origModel, user, trainMat, nItems, uInvalItems);
      auto itemSVDScoresPair =
          itemSVDScores(svdModel, user, trainMat, nItems, uInvalItems);
      auto itemFreqScoresPair = itemFreqScores(
          fullModel, origModel, itemFreq, user, trainMat, nItems, uInvalItems);
      auto itemOptScoresPair = itemOptScores(
          origModel, fullModel, user, data.trainMat, nItems, uInvalItems);
      ...
      updateMisPredFreqBins(misPredFreqCountBins, freqScoreBins,
                            itemOrigScoresPair, itemPredTopN, topBuckN,
                            itemFreq, user);

      auto gtNotInPred =
          orderingDiff(itemPredScoresPair, itemOrigScoresPair, topBuckN);
      updateMisPredBins(misGTSVDCountBins, misGTSVDScoreBins,
                        itemPredScoresPair, itemOrigTopN, topBuckN, svdModel,
                        user);
      ...
      auto freqInTopOrig =
          orderingOverlap(itemOrigScoresPair, itemFreqScoresPair, topBuckN);
      auto svdInTopOrig =
          orderingOverlap(itemOrigScoresPair, itemSVDScoresPair, topBuckN);
      ...
      freqSVDTopOvrlap +=
          compOrderingOverlap(itemFreqScoresPair, itemSVDScoresPair, topBuckN);
      freqTopRMSE +=
          sqrt(getSE(user, itemFreqScoresPair, origModel, fullModel, topBuckN) /
               topBuckN);
      svdTopRMSE +=
          sqrt(getSE(user, itemSVDScoresPair, origModel, fullModel, topBuckN) /
               topBuckN);
      ...
      rmse_SVDTopNOTInFreqTop +=
          sqrt(seSvdTopNotInFreqTop / svdTopNotInFreqTop);

      compMovAvg3(uSlidingAvgSVDRMSEs, uSlidingAvgSVD, uSlidingAvgSVD_freqs,
                  uSlidingAvgSVD_gt, itemSVDScoresPair, itemFreqMap, user,
                  nItems, slidingWidth, origModel, fullModel);
      compMovAvg3(uSlidingAvgFreqRMSEs, uSlidingAvgFreq, uSlidingAvgFreq_freqs,
                  uSlidingAvgFreq_gt, itemFreqScoresPair, itemFreqMap, user,
                  nItems, slidingWidth, origModel, fullModel);

      std::map<int, double> pprMap;
      if (NULL != graphMat) {
        itemPPRScoresPair = itemGraphItemScores(
            user, graphMat, trainMat, 0.01, nUsers, nItems, uInvalItems, false);
        for (auto &&itemScore : itemPPRScoresPair) {
          if (std::isnan(itemScore.second)) {
            std::cerr << "Found NaN: " << itemScore.first << " "
                      << itemScore.second << std::endl;
        ...
        auto pprInTopOrig =
            orderingOverlap(itemOrigScoresPair, itemPPRScoresPair, topBuckN);
        ...
        pprFreqTopOvrlap += compOrderingOverlap(itemPPRScoresPair,
                                                itemFreqScoresPair, topBuckN);
        pprSVDTopOvrlap +=
            compOrderingOverlap(itemPPRScoresPair, itemSVDScoresPair, topBuckN);
        pprTopRMSE += sqrt(
            getSE(user, itemPPRScoresPair, origModel, fullModel, topBuckN) /
            topBuckN);
        ...
        for (int i = 0; i < topBuckN; i++) {
          ...
          if (topFreqItems.find(item) != topFreqItems.end()) {
            ...
            score = origModel.estRating(user, item) -
        ...
        if (pprTopNotInFreqTop > 0) {
          ...
          rmse_PPRTopNOTInFreqTop +=
              sqrt(sePprTopNotInFreqTop / pprTopNotInFreqTop);
        }

        compMovAvg3(uSlidingAvgPPRRMSEs, uSlidingAvgPPR, uSlidingAvgPPR_freqs,
                    uSlidingAvgPPR_gt, itemPPRScoresPair, itemFreqMap, user,
                    nItems, slidingWidth, origModel, fullModel);
        if (1 == nThreads) {
          writeItemFreqSE(user, itemPPRScoresPair, itemFreqMap, origModel,
                          fullModel, uPPR_ItemFile, uPPR_FreqFile, uPPR_SEFile);
          writeItemFreqSE(user, itemSVDScoresPair, itemFreqMap, origModel,
                          fullModel, uSVD_ItemFile, uSVD_FreqFile, uSVD_SEFile);
          writeItemFreqSE(user, itemFreqScoresPair, itemFreqMap, origModel,
                          fullModel, uFreq_ItemFile, uFreq_FreqFile,
                          uFreq_SEFile);
        }
      }
      ...
      svdOrigOverlap +=
          compOrderingOverlap(itemOrigScoresPair, itemSVDScoresPair, topBuckN);
      freqOrigOverlap +=
          compOrderingOverlap(itemOrigScoresPair, itemFreqScoresPair, topBuckN);
      svdPredOverlap +=
          compOrderingOverlap(itemSVDScoresPair, itemPredScoresPair, topBuckN);
      ...
      if (NULL != graphMat) {
        // get itemsRMSE and itemsScore
        itemRMSEsOrdByItemScores(user, filtItems, fullModel, origModel,
                                 itemsRMSE, itemsScore, itemPPRScoresPair);
        // reset user specific vec to 0
        std::fill(uBucketNNZ.begin(), uBucketNNZ.end(), 0);
        std::fill(uRMSEPPRScores.begin(), uRMSEPPRScores.end(), 0);
        updateBucketsArr(uRMSEPPRScores, uBucketNNZ, itemsRMSE, nBuckets);
      ...
      // get signerror
      itemSignEsOrdByItemScores(user, filtItems, fullModel, origModel,
                                itemsRMSE, itemsScore, itemOrigScoresPair);
      // reset user specific vec to 0
      std::fill(uBucketNNZ.begin(), uBucketNNZ.end(), 0);
...
#pragma omp critical
  ...
  opFile << "predOrigOverlap: " << predOrigOverlap / totalSampUsers
         << std::endl;
  opFile << "svdOrigOverlap: " << svdOrigOverlap / totalSampUsers << std::endl;
  opFile << "freqOrigOverlap: " << freqOrigOverlap / totalSampUsers
         << std::endl;
  ...
  opFile << "freqSVDTopOvrlap: " << freqSVDTopOvrlap / totalSampUsers
         << std::endl;
  opFile << "pprFreqTopOvrlap: " << pprFreqTopOvrlap / totalSampUsers
         << std::endl;
  opFile << "pprSVDTopOvrlap: " << pprSVDTopOvrlap / totalSampUsers
         << std::endl;
  ...
  opFile << "pprofPredInOrig: " << pprOfPredInOrig / totalSampUsers
         << std::endl;
  opFile << "pprOfPredNotInOrig: " << pprOfPredNotInOrig / totalSampUsers
         << std::endl;
  ...
  opFile << "svdOfPredInOrig: " << svdOfPredInOrig / totalSampUsers << std::endl
         << "svdOfPredNotInOrig: " << svdOfPredNotInOrig / totalSampUsers
         << std::endl;

  opFile << "svdVarOfPredInOrig: " << svdVarOfPredInOrig / totalSampUsers
         << std::endl
         << "svdVarOfPredNotInOrig: " << svdVarOfPredNotInOrig / totalSampUsers
         << std::endl;

  opFile << "svdAboveAvgInOrig: " << svdAboveAvgInOrig / totalSampUsers
         << std::endl;

  opFile << "svdOfMaxPredInorig: " << svdOfMaxPredInorig / totalSampUsers
         << std::endl;
  opFile << "svdOfMinPredInOrig: " << svdOfMinPredInOrig / totalSampUsers
         << std::endl;
  opFile << "svdOfTopPredInOrig: " << svdOfTopPredInOrig / totalSampUsers
         << std::endl;
  opFile << "svdOfMedPredInOrig: " << svdOfMedPredInOrig / totalSampUsers
         << std::endl;
  opFile << "svdOfBotPredInOrig: " << svdOfBotPredInOrig / totalSampUsers
         << std::endl;

  opFile << "pprOfPredInOrig: " << pprOfPredInOrig / totalSampUsers << std::endl
         << "pprOfPredNotInOrig: " << pprOfPredNotInOrig / totalSampUsers
         << std::endl;

  opFile << "iterPredSVDOrigOverlap: "
         << iterPredSVDOrigOverlap / totalSampUsers << std::endl;

  opFile << "freq_SVDTopInFreqTop: " << freq_SVDTopInFreqTop / totalSampUsers
         << std::endl;
  opFile << "freq_SVDTopNOTInFreqTop: "
         << freq_SVDTopNOTInFreqTop / totalSampUsers << std::endl;
  opFile << "rmse_SVDTopInFreqTop: " << rmse_SVDTopInFreqTop / totalSampUsers
         << std::endl;
  opFile << "rmse_SVDTopNOTInFreqTop: "
         << rmse_SVDTopNOTInFreqTop / totalSampUsers << std::endl;

  opFile << "freq_PPRTopInFreqTop: " << freq_PPRTopInFreqTop / totalSampUsers
         << std::endl;
  opFile << "freq_PPRTopNOTInFreqTop: "
         << freq_PPRTopNOTInFreqTop / totalSampUsers << std::endl;
  opFile << "rmse_PPRTopInFreqTop: " << rmse_PPRTopInFreqTop / totalSampUsers
         << std::endl;
  opFile << "rmse_PPRTopNOTInFreqTop: "
         << rmse_PPRTopNOTInFreqTop / totalSampUsers << std::endl;
  ...
  opFile << "Train RMSE: " << fullModel.RMSE(trainMat, invalUsers, invalItems)
         << std::endl;
  opFile << "Test RMSE: "
         << fullModel.RMSE(data.testMat, invalUsers, invalItems) << std::endl;
  opFile << "Val RMSE: " << fullModel.RMSE(data.valMat, invalUsers, invalItems)
         << std::endl;
  opFile << "Full RMSE: "
...
void predSampUsersRMSEFreqPar(const Data &data, int nUsers, int nItems,
                              Model &origModel, Model &fullModel,
                              std::unordered_set<int> &invalUsers,
                              std::unordered_set<int> &invalItems,
                              std::unordered_set<int> &filtItems,
...
std::map<int, std::vector<int>> pprSampTopNItemsUsers(
    gk_csr_t *graphMat, gk_csr_t *trainMat, int nUsers, int nItems,
    Model &origModel, Model &fullModel, float lambda, int max_niter,
    std::unordered_set<int> &invalUsers, std::unordered_set<int> &invalItems,
    std::unordered_set<int> &filtItems, int nSampUsers, int seed, int N,
    std::string &prefix) {
  ...
  std::string scoreFName =
      prefix + "_" + std::to_string(lambda) + "_ppr_bucket.txt";
  writeVector(bucketScores, scoreFName.c_str());

  // write out bucket nnz
  std::string nnzFName =
      prefix + "_" + std::to_string(lambda) + "_nnz_bucket.txt";
  writeVector(bucketNNZ, nnzFName.c_str());

  return itemUsers;
}

std::map<int, std::vector<int>> svdSampTopNItemsUsers(
    gk_csr_t *trainMat, int nUsers, int nItems, Model &origModel,
    Model &fullModel, Model &svdModel, std::unordered_set<int> &invalUsers,
...
std::map<int, double>
itemUsersRMSE(Model &origModel, Model &fullModel,
              std::map<int, std::vector<int>> &itemUsers) {
...
void writeTopBuckRMSEs(Model &origModel, Model &fullModel, Model &svdModel,
                       gk_csr_t *graphMat, gk_csr_t *trainMat, float lambda,
                       int max_niter, std::unordered_set<int> &invalUsers,
                       std::unordered_set<int> &invalItems,
                       std::unordered_set<int> &filtItems, int nSampUsers,
                       int seed, int N, std::string prefix) {
  ...
  */
  ...
  auto itemUsers = svdSampTopNItemsUsers(
      trainMat, fullModel.nUsers, fullModel.nItems, origModel, fullModel,
      svdModel, invalUsers, invalItems, filtItems, nSampUsers, seed, N, prefix);
  ...
  auto itemARMSE =
      itemAllRMSE(origModel, fullModel, items, invalUsers, invalItems);
  ...
  if (opFile.is_open()) {
    for (auto &&item : items) {
      opFile << item << " " << itemUsers[item].size() << " " << itemURMSE[item]
             << " " << itemARMSE[item] << std::endl;
    }
    opFile.close();
  }
}
