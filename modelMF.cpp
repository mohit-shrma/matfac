...
void ModelMF::hogSpTrain(const Data &data, Model &bestModel, 
    std::unordered_set<int>& invalidUsers,
  ...
  std::cout << "\nModelMF::hogSpTrain trainSeed: " << trainSeed;
  ...
  std::mt19937 mt(trainSeed);
  //get user-item ratings from training data
  auto uiRatings = origModel.getUIRatings(trainMat, invalidUsers, invalidItems);
...
#pragma omp parallel for
    ...
    if (iter % OBJ_ITER == 0 || iter == maxIter-1) {
      if (isTerminateModel(bestModel, data, iter, bestIter, bestObj, prevObj,
            invalidUsers, invalidItems, uiRatings)) {
        break; 
      }
